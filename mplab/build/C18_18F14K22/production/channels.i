...
# 52 "C:\Program Files (x86)\Microchip\xc8\v2.00\pic\include\pic18f14k22.h"
...
unsigned int timer[4 + 1];
unsigned char bitmask[4 + 1];
} CHANNELINFO;
...
# 14 "channels.c"
unsigned char channelLevel[4];

static unsigned char channelRank[4];
...
if ( i == 0 ) {
pCIWrite -> timer[i2] = channelLevel[pivot[i]] * 16;
}
else {
pCIWrite -> timer[i2] = (channelLevel[pivot[i]] - channelLevel[pivot[i-1]]) * 16;
}

pCIWrite -> bitmask[i2+1] = (1 << pivot[i]);

# 182
while ((i < (4 - 1)) && (channelLevel[pivot[i]] == channelLevel[pivot[i+1]] ) ) {
...
pCIWrite -> timer[i2] = (0x00ff * 16) - sumTimers;
